    ...
    return Column(children: [
      ...
      Expanded(
        child: asyncUsers.when(
          data: (users) {
            if (users.isEmpty) return Center(child: Padding(padding: const EdgeInsets.all(32), child: NeuCard(padding: const EdgeInsets.all(24), child: Column(children: [Icon(Icons.people_outline, size: 40, color: AppColors.inkSoft), const SizedBox(height: 12), Text('No students found', style: AppTypography.soraHeading3()), Text('Try different filters', style: AppTypography.interCaption())]))));
    ...
    if (_pending.isEmpty) {
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [NeuCard(padding: const EdgeInsets.all(24), child: Column(children: [Icon(Icons.inbox_outlined, size: 40, color: AppColors.inkSoft), const SizedBox(height: 12), Text('No pending requests', style: AppTypography.soraHeading3()), Text('When someone sends you a request, it will appear here.\nYou can Accept or Reject.', style: AppTypography.interCaption(), textAlign: TextAlign.center)]))],
        ),
      );
    }
    return RefreshIndicator(
...
class _StudentCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.only(bottom: 12), child: NeuCard(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [UserAvatar(name: student.name, size: 50, isVerified: student.isVerified, isPremium: student.isPremium, imageUrl: student.photoUrl), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Flexible(child: Text(student.name, style: AppTypography.interButton(color: AppColors.ink, size: 15))), if (student.isPremium) ...[const SizedBox(width: 6), const PremiumBadge(isSmall: true)]]), const SizedBox(height: 2), Text('${student.college ?? '—'} • ${student.branch ?? '—'}', style: AppTypography.interBodySmall(), overflow: TextOverflow.ellipsis), Text('${student.city ?? ''}, ${student.state ?? ''} • Year ${student.yearOfStudy ?? '-'}', style: AppTypography.interCaption())])), _ConnectButton(state: connectionState, onTap: onConnect)]), if (student.skills.isNotEmpty) ...[const SizedBox(height: 12), Wrap(spacing: 6, runSpacing: 6, children: student.skills.take(4).map((s) => NeuChip(label: s)).toList())], const SizedBox(height: 8), Text('${student.connectionsCount} connections', style: AppTypography.interCaption(color: AppColors.cyanDeep))]))));
}
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  @override
  Widget build(BuildContext context) => SafeArea(top: false, child: Container(margin: const EdgeInsets.fromLTRB(16, 16, 16, 16 + 72), padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: AppColors.bg, borderRadius: BorderRadius.circular(28), boxShadow: AppColors.neuRaisedShadows), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: AppColors.shadowDark, borderRadius: BorderRadius.circular(2)))), const SizedBox(height: 16), Text('Filter Students', style: AppTypography.soraHeading3()), const SizedBox(height: 4), Text('Filters apply instantly on search + Discover API', style: AppTypography.interCaption()), const SizedBox(height: 16), Text('State', style: AppTypography.interLabel()), const SizedBox(height: 8), Wrap(spacing: 8, runSpacing: 8, children: _states.map((s) => NeuChip(label: s, isSelected: _state == s, onTap: () => setState(() => _state = _state == s ? null : s))).toList()), const SizedBox(height: 16), Text('Year of Study', style: AppTypography.interLabel()), const SizedBox(height: 8), Wrap(spacing: 8, children: [1, 2, 3, 4, 5].map((y) => NeuChip(label: 'Year $y', isSelected: _year == y, onTap: () => setState(() => _year = _year == y ? null : y))).toList()), const SizedBox(height: 16), Text('Branch', style: AppTypography.interLabel()), const SizedBox(height: 8), Wrap(spacing: 8, runSpacing: 8, children: ['Computer Science', 'Electronics', 'Mechanical', 'Data Science', 'Economics', 'MBA'].map((b) => NeuChip(label: b, isSelected: _branch == b, onTap: () => setState(() => _branch = _branch == b ? null : b))).toList()), const SizedBox(height: 24), Row(children: [Expanded(child: GestureDetector(onTap: () => widget.onApply(null, null, null), child: const NeuCard(padding: EdgeInsets.symmetric(vertical: 14), child: Center(child: Text('Clear'))))), const SizedBox(width: 12), Expanded(child: GestureDetector(onTap: () => widget.onApply(_state, _year, _branch), child: Container(height: 50, decoration: BoxDecoration(gradient: AppColors.cyanGradient, borderRadius: BorderRadius.circular(16)), alignment: Alignment.center, child: Text('Apply', style: AppTypography.interButton(color: Colors.white)))))]), const SizedBox(height: 8)]))));
}
